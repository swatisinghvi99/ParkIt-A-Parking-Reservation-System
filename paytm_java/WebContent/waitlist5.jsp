...
<%
    ...
    try{
        ...
        if(k!=0){
            ...
            ps3.executeUpdate();
            ps4.executeUpdate();
            PreparedStatement ps = conn.prepareStatement("INSERT INTO waitlist (orderid, custid, fullName, vehicle_no, city, location, locationType, date, in_time, duration, out_time, vehicleType, amount, waitlist, paid_status, status) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
            ...
            if(x>0){
                session.setAttribute("ord",orderid); 
                session.setAttribute("cust",custid);
                response.sendRedirect("waitlist6.jsp");
            }
            else{
    %>
                <script> 
